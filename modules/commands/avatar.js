  ...
  module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
    ...
    switch (handleReply.type) {
      case "characters": {
        ...
        const res = await axios.get(`https://api.nguyenmanh.name.vn/taoanhdep/search/id?id=${id}`);
  
        var name = res.data.name
        
        api.unsendMessage(handleReply.messageID);
        return api.sendMessage(`» Bạn đã chọn ID nhân vật là ${name}\n» Reply tin nhắn này và nhập chữ nền của bạn`, threadID, (error, info) => {
          return global.client.handleReply.push({
            type: 'chunen',
            name: this.config.name,
            author: senderID,
      ...
      case "chunen": {
        api.unsendMessage(handleReply.messageID);
        return api.sendMessage(`» Reply tin nhắn này để nhập chữ kí của bạn`, threadID, (error, info) => {
          return global.client.handleReply.push({
            type: 'chuki',
            name: this.config.name,
            author: senderID,
      ...
      case "create": {
        ...
        var chuki_ = handleReply.chuki;
        var chunen_ = handleReply.chunen;
        var color_ = event.body;
        api.unsendMessage(handleReply.messageID);
        return api.sendMessage(`Đang tạo ảnh ...`, event.threadID, async (error, info) => {
          await new Promise(resolve => setTimeout(resolve, 3 * 1000));
          var imag = (await axios.get(`https://api.nguyenmanh.name.vn/taoanhdep/avatarwibu?id=${idchar}&chu_nen=${encodeURIComponent(chunen_)}&chu_ky=${encodeURIComponent(chuki_)}&color=${encodeURIComponent(color_)}&apikey=KeyTest`, {
            responseType: "stream"
          })).data;
          var msg = {
