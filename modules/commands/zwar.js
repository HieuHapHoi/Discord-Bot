...
module.exports.run = async function({ api, event, args, client, Currencies, Users }) {
    ...
    switch (args[0]) {
        ...
        case "shop": {
            if (Object.entries(dataUser).length == 0)return api.sendMessage(`[Chiến Trường] Bạn cần đăng ký vào chiến trường, hãy sử dụng '[ ${prefix}zwar register ]'`, event.threadID, event.messageID);
            return api.sendMessage(
                "[ ❖ ] [ = [ Shop Weapon ] = ] [ ❖ ]" +
                "\n[ ❖ ] Hãy Nhập Lựa Chọn [ ❖ ]" +
                "\n[ ❖ ] Mua Súng [ 1 ] [ ❖ ]" +
