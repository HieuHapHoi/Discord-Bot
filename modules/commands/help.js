 ...
 module.exports.run = function({ api, event, args, getText }) {
  ...
  if (args[0] == "all") {
    ...
    let callback = function () {
        ...
        }; request(res.data.url).pipe(fs.createWriteStream(__dirname + `/cache/helpall.${ext}`)).on("close", callback);
     })
};
    ...
    api.sendMessage({body: msg + hi, attachment: fs.createReadStream(__dirname + `/cache/help.${ext}`)
        ...
        }; request(res.data.url).pipe(fs.createWriteStream(__dirname + `/cache/help.${ext}`)).on("close", callback);
     })
}
...
» HDSD: ${prefix}${command.config.name} ${(command.config.usages) ? command.config.usages : "Không có hướng dẫn"}`,
        ...
        }; request(res.data.url).pipe(fs.createWriteStream(__dirname + `/cache/helpin4.${ext}`)).on("close", callback);
     })
};